...
router.post("/register", async (req, res) => {
  try {
    ...
    return res.status(201).json({
      ...
      token,
      user: { id: userId, fullName, username, email, hcode, isVerified: false },
    });
  } catch (err) {
...
router.post("/verify-email", async (req, res) => {
  try {
    ...
    if (user.verificationExp.toDate() < new Date()) {
    ...
    return res.json({ success: true, message: "Email verified! You are all set." });
  } catch (err) {
